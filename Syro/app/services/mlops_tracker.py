...
class MLOpsTracker:
        ...
        if tracking_uri:
            ...
            if sys.platform == "win32":
                mlflow.set_tracking_uri(
                    f"file:///{str(mlflow_dir.absolute()).replace(chr(92), '/')}"
                )
            else:
        ...
        **kwargs,
        ...
        try:
            ...
            with mlflow.start_run(run_name=f"rag_query_{int(time.time())}"):
                ...
                query_file = (
                    Path(settings.data_dir) / "tmp" / f"query_{int(time.time())}.txt"
                )
    ...
    def log_document_ingestion(
        ...
        try:
            ...
            with mlflow.start_run(run_name=f"ingestion_doc_{document_id}"):
                ...
                mlflow.log_metric(
                    "avg_chunk_size", document_size_chars / max(num_chunks, 1)
                )
    ...
    def log_warning(
        ...
        try:
            ...
            with mlflow.start_run(
                run_name=f"warning_{warning_type}_{int(time.time())}"
            ):
    ...
    def get_latest_metrics(
        self, limit: int = 10, include_errors: bool = True
    ) -> list[dict[str, Any]]:
