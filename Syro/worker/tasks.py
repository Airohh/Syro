...
def process_document_upload(
    ...
    try:
        logger.info(
            f"Début ingestion document {document_id} (org: {organization_id}, domain: {domain})"
        )
        ...
        try:
            ...
            document_ingestions_total.labels(
                domain=domain_str, status="processing"
            ).inc()
        ...
        try:
            ...
            document_ingestion_duration_seconds.labels(domain=domain_str).observe(
                duration
            )
        ...
        logger.info(
            f"Ingestion terminée pour document {document_id}: {chunk_count} chunks en {duration:.2f}s"
        )
        ...
        duration = time.time() - start_time
        logger.error(
            f"Erreur lors de l'ingestion du document {document_id}: {str(exc)}",
            exc_info=True,
        )
        ...
        try:
            ...
            logger.error(
                f"Erreur lors de la mise à jour du statut d'erreur: {str(db_error)}"
            )

        # Retry automatique si ce n'est pas la dernière tentative
        if self.request.retries < self.max_retries:
            logger.info(
                f"Retry {self.request.retries + 1}/{self.max_retries} pour document {document_id}"
            )
